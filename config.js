...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
        ...
        if (isGroupMsg && isxp && !isWin(user) && !isBlocked) {
            try {
                ...
                if (neededXp <= getXp(user, nivel)) {
					...
					// Ative isso para fazer a Taiga mandar mensagem de level UP, todavia vai atrapalhar em casos de muito xp ganho nos jogos e steal
		...
		case 'about':
			await kill.sendFile(from, 'https://i.imgur.com/2p7XLPb.png', 'iris.png', mess.about(), id)
			break
       ...
       case 'translate':
		...
		// Se quiser adicione respostas na reply.txt ou use o comando '/resposta', taiga também consegue adicionar ela mesma sozinha
        case 'taiga':
			try {
				if (args[0] == '-g') {
				...
				await kill.reply(from, chatBotR, id)
				console.log(color('[TAIGA]', 'crimson'), color(`→ Obtive erros no comando ${prefix}${command} → ${error.message} - Você pode ignorar.`, 'gold'))
			}
			break
       ...
       case 'clima':
		...
		if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
			...
			await kill.reply(from, mess.soademiro(), id)
					...
					break
					case 'calendario':
					await kill.sendFile(from, './lib/media/img/calendario.jpg', 'calendario.jpg', 'ta aqui bem', id)
					break

					case 'horarios':
								case 'horario':
					case 'horários':
					await kill.sendFile(from, './lib/media/img/cronograma.jpg', 'cronograma.jpg', 'amém', id)
					break
...
break
        ...
        case 'antitravas':
            ...
            break
						...
						case 'destrava':
								await kill.sendText(from, mess.destrava(), id)
							break
							case 'destravar':

								await kill.sendText(`${args[0]}` + '@c.us', mess.avisodes())
				await kill.sendText(`${args[0]}` + '@c.us', mess.destrava())
				await kill.sendText(`${args[0]}` + '@c.us', mess.destrava())
				await kill.sendText(`${args[0]}` + '@c.us', mess.destrava())
				await kill.sendText(`${args[0]}` + '@c.us', mess.destrava())
				await kill.sendText(`${args[0]}` + '@c.us', mess.destrava())
				...
				await kill.sendText(`${args[0]}` + '@c.us', mess.destrava())
				await kill.sendText(`${args[0]}` + '@c.us', mess.destrava())
								await kill.reply(from, mess.makedestrava(), id)
						break
						case 'tarefas':
						await kill.sendFile(from, 'https://nurutomo.herokuapp.com/api/ssweb?url=https%3A%2F%2Fpadlet.com%2Fpedrohenriqueoreis%2Fr3z3j1ezdqni0eb7&full=false&delay=1000&type=jpeg&quality=1.0', 'tarefa.jpeg', 'mais em https://cutt.ly/qbXdGZb', id)
						break
						//https://docs-jojo.herokuapp.com/api/ssweb_full?url=https://padlet.com/pedrohenriqueoreis/r3z3j1ezdqni0eb7&device=pc
						//https://nurutomo.herokuapp.com/api/ssweb?url=https%3A%2F%2Fpadlet.com%2Fpedrohenriqueoreis%2Fr3z3j1ezdqni0eb7&full=false&delay=1000&type=jpeg&quality=1.0
